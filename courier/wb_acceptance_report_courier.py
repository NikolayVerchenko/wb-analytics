...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch Wildberries acceptance report into raw tables."
    )
...
def extract_task_id(payload: dict) -> str:
    ...
    if not task_id:
        raise RuntimeError("Missing data.taskId in acceptance report create response")
    return str(task_id)
...
def extract_status(payload: dict) -> str:
    ...
    if not status:
        raise RuntimeError("Missing data.status in acceptance report status response")
    return str(status)
