...
def run() -> int:
    ...
    with db_connection() as conn:
        ...
        try:
            ...
            runner.start()
            response = client.get(ENDPOINT, expected_statuses={200})
            payload = response.json()
            if not isinstance(payload, dict):
                raise RuntimeError(f"Expected object payload, got {type(payload).__name__}")
            insert_raw_payload(conn, load_id=load_id, account_id=account_id, source=SOURCE, request_params={}, payload=payload, period_mode=None, week_start=None)
            runner.succeed(1)
